...
public class EditServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try{
            if(verifyCarInfo(car)){
                if(CarDao.findByPlateNo(car.getPlateNo())==null){
                        throw new IllegalArgumentException("The Car  With PlateN0:"+car.getPlateNo()+"Does Not exist ");
    ...
    private boolean verifyCarInfo(Car car){
           ...
           if(!car.getPlateNo().matches("[GP||GR]{2}[0-9]{3}[A-Z]{1}")){
               errors.put("plateNo","PlateNo Must Begin with GP OR GR AND COMPOSED BY 3 DIGITS WITH ALPHABET AT THE END LIKE THIS GR000Q");
